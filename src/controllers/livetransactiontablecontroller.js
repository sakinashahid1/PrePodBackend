  ...
  try {
    ...
    const response = await fetch(
      `https://bin-ip-checker.p.rapidapi.com/?bin=${bin}`,
      {
        ...
        headers: {
          "x-rapidapi-key":
            "4403aa3601msh220f59b21e3c805p19213fjsn4267c5dcd8f3",
    ...
    return [
      data.BIN["scheme"],
      data.BIN.country["name"],
      data.BIN.country["alpha2"],
    ];
  } catch (error) {
...
async function fetchFromNeutrinoAPI(bin) {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Neutrino API response was not ok: ${response.statusText}`
      );
    }
...
async function binAPI(req, res) {
  const { bin } = req.body;
  if (typeof bin === "string" && bin.length === 6 && /^\d{6}$/.test(bin)) {
    try {
      let result;
      if (apiToggle) {
      ...
      try {
        let fallbackResult;
        if (!apiToggle) {
          // Flip the logic to use the opposite API of the failed one
        ...
        const formattedCountryName =
          fallbackResult[1].charAt(0).toUpperCase() +
          fallbackResult[1].slice(1).toLowerCase();
    ...
    return res
      .status(400)
      .json({ code: 400, status: "Bad request", error: "Invalid BIN format" });
  }
}
...
async function getLivedata(req, res) {
  try {
    ...
    for (const item of data) {
      const uniqueKey = `${item.transactionId}-${item.mtxnID}-${item.status}`;

      if (!seenTransactions.has(uniqueKey)) {
        seenTransactions.add(uniqueKey);
        if (item.id > maxId) {
          const bin = item.cardNo.slice(0, 6);
        ...
        const binResult = await response.json();
        our_country = binResult.data.country;
        our_cardType = binResult.data.cardType;
          newRecords.push({
            livedata_id: item.id,
...
async function fetchTransactionsAndUpdate(req, res) {
  ...
  try {
    ...
    const bulkOps = uniqueTransactions.map((item) => ({
      updateOne: {
        filter: { livedata_id: item.id },
        update: {
          $set: {
...
async function load7daysData(req, res) {
  try {
    const endDate = new Date();
    const startDate = new Date();
    startDate.setDate(endDate.getDate() - 6);

    const formattedStartDate = `${startDate.getFullYear()}-${("0" + (startDate.getMonth() + 1)).slice(-2)}-${("0" + startDate.getDate()).slice(-2)} 00:00:00`;
    const formattedEndDate = `${endDate.getFullYear()}-${("0" + (endDate.getMonth() + 1)).slice(-2)}-${("0" + endDate.getDate()).slice(-2)} 23:59:59`;

    console.table([formattedStartDate, formattedEndDate]);

    req.body.fromDate = formattedStartDate;
    req.body.toDate = formattedEndDate;

    await fetchTransactionsAndUpdate(req, res);
  } catch (error) {
    console.error("Error in load7daysData:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
}
...
async function bankSettled(req, res) {
  ...
  const txnidArray = txnids
    .split(/\s+/)
    .map((txnid) => txnid.trim())
    .filter((txnid) => txnid);

  if (txnidArray.length === 0) {
    return res
      .status(400)
      .json({ message: "Invalid input. Please provide at least one txnid." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while updating transactions" });
  }
}
